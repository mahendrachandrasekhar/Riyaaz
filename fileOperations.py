...
def writeEmptyNote(xmlHandle,lookupTable,myspeed):
    ...
    xmlHandle.write(myStrToWrite.replace("begin","end").replace("eighth",myspeed))
    xmlHandle.write('\n')
    
def findAndWriteCurrentNoteWithPrefix(myRaag,currPattern,Aaroh_Or_Avaroh,myStrToPrefix,myRaagaTable):
    myNote = ((myRaag.loc[(CurrentRaag['Sequence'] == currPattern) & (myRaag['Aaroh_Avaroh'] == Aaroh_Or_Avaroh)])['LyricalNote']).to_string(index=False)
    myXML = myRaagaTable.loc[(myRaagaTable['LyricalNote'] == myNote) & (myRaagaTable['Scale'] == Scale)]
    myStrToWrite = (myXML['XMLNotation']).to_string(index=False)
    fout.write(myStrToPrefix.replace("eighth",speed) + myStrToWrite.replace("begin","end").replace("eighth",speed))
